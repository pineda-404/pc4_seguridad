...
Esta simplificación reduce drásticamente el fan-out y el número de compuertas XOR necesarias para la difusión lineal. En la Fig. \ref{fig:estructura} se detalla este flujo.
...
\midrule
B-r1-z1 & Baseline (Keccak) & 1 & 70 & 387 & 1 (Óptimo) & $2^{-2}$ & $2^2$ & 0.99 \\
B-r2-z1 & Baseline (Keccak) & 2 & 115 & 772 & 2 (Óptimo) & $2^{-4}$ & $2^4$ & 3.11 \\
B-r3-z1 & Baseline (Keccak) & 3 & 160 & 1157 & 3 (Óptimo) & $2^{-6}$ & $2^6$ & 10.27 \\
\midrule
M-r1-z1 & Propuesta ($\Sigma''_{\text{ligero}}$) & 1 & 165 & 797 & 2 (Óptimo) & $2^{-4}$ & $2^4$ & 2.67 \\
M-r2-z1 & Propuesta ($\Sigma''_{\text{ligero}}$) & 2 & 305 & 1592 & 4 (Óptimo) & $2^{-8}$ & $2^8$ & 8.81 \\
M-r3-z1 & Propuesta ($\Sigma''_{\text{ligero}}$) & 3 & 445 & 2387 & 6 (Óptimo) & $2^{-12}$ & $2^{12}$ & 31.95 \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{itemize}
    ...
    \item \textbf{En la Propuesta:} La inyección lineal de paridad es directa ($S[x,y] \oplus P_c[x]$). Un estado de una sola fila activa causaría una dispersión lineal a las otras 4 filas. Para evitar esta difusión y cancelar el término $P_c[x]$, el modelo se ve obligado a activar un número par de filas uniformes. El mínimo par estable es exactamente **2 filas**, lo que fuerza el óptimo de 2 S-boxes activas por ronda.
\end{itemize}
...
Analizamos tres vulnerabilidades críticas identificadas en auditorías previas:

\subsection{V1: Vulnerabilidad de Bloques Diagonales}
Al eliminar la paridad de filas en $\Sigma''_{\text{ligero}}$, la difusión horizontal se reduce. No obstante, la estructura permutativa de $\pi$ y los offsets de $\rho$ (incluso degenerados a modulo 1) rompen la formación de sub-bloques independientes a lo largo de las rondas, garantizando que el estado completo se mezcle en $R \ge 2$.

\subsection{V2: Canal Lateral de Tiempo por Regla de Piso}
La inyección de constantes o alineación lógica no lineal podría inducir dependencias de datos en el tiempo de cómputo del solver. En el entorno real, todas las operaciones de la propuesta (XORs y NOT-ANDs) se ejecutan en tiempo constante bit-sliced en hardware, eliminando fugas temporales de canal lateral.
...
La permutación Keccak modificada con $\Sigma''_{\text{ligero}}$ y S-box ASCON $\chi'$ ofrece una alternativa robusta para criptografía ligera en IoT. Bajo un modelado MILP riguroso y cryptográficamente coherente ($t \le a+b$), se demostró que la propuesta duplica el margen de seguridad frente a ataques diferenciales en comparación con el baseline a escala degenerada ($z=1$). A pesar del incremento en el consumo de compuertas XOR en hardware por el pre/post-mix de ASCON, el diseño reduce la complejidad de ruteo al remover la paridad cruzada de fila. El trabajo futuro se centrará en extender el modelado MILP a escalas reales $z \ge 8$ empleando solvers heurísticos paralelos para mitigar la intratabilidad del problema discreto.
