...
A escala degenerada $z=1$, $\Sigma''_{\text{ligero}}$ requiere únicamente 45 compuertas XOR frente a las 50 compuertas de la capa $\theta$ estándar (una reducción neta del 10\% en lógica de mezcla lineal). Para escalas reales $z > 1$, la mezcla activa intra-lane por XOR incrementa el conteo absoluto de compuertas a $95z$ XORs (frente a $50z$ de Keccak $\theta$). No obstante, este coste adicional se justifica por la eliminación completa de cableado y mezcla diagonal cruzada entre columnas vecinas, reduciendo drásticamente la complejidad física de ruteo bidimensional y el fan-out en el silicio. En la Fig. \ref{fig:estructura} se detalla este flujo.
...
\subsection{V2: Resistencia contra Canales Laterales de Potencia (DPA) mediante Enmascaramiento}
En implementaciones físicas para IoT, la resistencia contra ataques de análisis de potencia diferencial (DPA) es crítica. El reemplazo de la S-box $\chi$ de Keccak por la S-box $\chi'$ de ASCON se justifica por la compatibilidad de esta última con esquemas de enmascaramiento por hardware (threshold implementations). Debido al balance de las funciones coordenadas cuadráticas de $\chi'$, es posible implementar enmascaramientos de primer orden con un número reducido de acciones aleatorias adicionales (sharing), mitigando fugas físicas de canal lateral con menor sobrecoste de área frente a alternativas de mayor profundidad AND como PRESENT o GIFT.
